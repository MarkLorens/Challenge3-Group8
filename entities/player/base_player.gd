...
=======
	...
	if SaveManager.has_save():
		...
		if saved_grid != Vector2i(-1, -1):
			...
			SaveManager.delete_save()  # ← pindah ke luar to_global()
	else:
		current_grid = floor_tilemap.local_to_map(  # ← kurung buka di baris yang sama
			floor_tilemap.to_local(global_position)
		)
	
	await get_tree().process_frame
	highlight_layer.show_move_range(current_grid, max_move_distance)  # ← pakai tab, bukan spasi
>>>>>>> Stashed changes
